...
class Falkon():
    ...
    def update(self, K_mm_lz, n,m, lambd):
        ...
        shift = 10**-16*torch.trace(K_mm)

        # Get preconditioning matrices via Cholesky factorization 
        T = torch.linalg.cholesky(K_mm+shift*torch.eye(m).to(self.device),upper = True)
        R = torch.linalg.cholesky(n/m*(T@T.T)+lambd*torch.eye(m).to(self.device),upper = True)
