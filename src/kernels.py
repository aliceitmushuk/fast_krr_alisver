...
def _get_matern_kernel(x1_lazy, x2_lazy, sigma, nu):
    ...
    if nu == 0.5:
        ...
        K = (1 + torch.sqrt(torch.tensor(3.0)) * D / sigma) * (
            -(torch.sqrt(torch.tensor(3.0)) * D / sigma)
        ).exp()
    else:  # nu == 2.5
        K = (
            1 + torch.sqrt(torch.tensor(5.0)) * D / sigma + 5 * D**2 / (3 * sigma**2)
        ) * (-(torch.sqrt(torch.tensor(5.0)) * D / sigma)).exp()
...
def _get_kernel(x1_lazy, x2_lazy, kernel_params):
    ...
    if kernel_params["type"] == "rbf":
        ...
        return _get_matern_kernel(
            x1_lazy, x2_lazy, kernel_params["sigma"], kernel_params["nu"]
        )
