...
from opt_utils_pcg import _get_precond, _get_precond_inducing, _get_kernel_matrices, _get_kernel_matrices_inducing, _init_pcg,_step_pcg 
...
class PCG():
    ...
    def run(self, 
      ...
      if inducing:
            ...
            inducing_pts = torch.from_numpy(np.random.choice(a0.shape[0],kernel_params['m'],replace = False))
            K_mm, K_nm, K_tst, m, n, b_norm = _get_kernel_matrices_inducing(x,x_tst,inducing_pts,kernel_params,b)
            precond = _get_precond_inducing(K_mm,n,m,lambd,self.precond_params,device)
            def K_Lin_Op(v): return K_nm.T@(K_nm@v)+lambd*(K_mm@v)
            Knm_Tb = K_nm.T@b
      ...
      for i in range(max_iter):
           ...
           if logger_enabled:
                if inducing:
                   ...
                   logger.compute_log_reset(K_Lin_Op, K_tst, a, Knm_Tb, b_tst, b_norm, task, i, True)
