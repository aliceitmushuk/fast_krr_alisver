...
with h5py.File(os.path.join(data_folder, 'full_data.h5py'), 'w', libver='latest') as f:
    Xdset = f.create_dataset("X", (total_rows, dim), dtype='float64',
    ...
    for X, Y in zip(X_list, Y_list):
        ...
        Y = Y.reshape((-1, 1))
        Xdset.write_direct(X, dest_sel=np.s_[
                           current_i:current_i+X.shape[0], :])
        Ydset.write_direct(Y, dest_sel=np.s_[
                           current_i:current_i+Y.shape[0], :])
        current_i += X.shape[0]
        print("i: %d/%d in %.2fs" % (current_i, total_rows, time.time() - t_s))

# Open the big .h5py file for subsampling
with h5py.File(os.path.join(data_folder, "full_data.h5py"), 'r') as f:
    print(f.keys())
...
with h5py.File(os.path.join(data_folder, 'subsampled_data.h5py'), 'w', libver='latest') as f:
    Xdset = f.create_dataset("X", (n_samples, dim), dtype='float64',
                             compression="gzip", chunks=(chunk_x, dim))
    ...
    Xdset.write_direct(X_small, dest_sel=np.s_[
        current_i:current_i+X_small.shape[0], :])
    Ydset.write_direct(y_small, dest_sel=np.s_[
        current_i:current_i+y_small.shape[0], :])
